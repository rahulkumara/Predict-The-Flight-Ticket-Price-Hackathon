...
library(readxl)
data = read_excel("Data_Train.xlsx")
...
targetdata = ad[,-c(1,2,3,4,5,6,7,8,10,13,14,15,17,18)]
...
hist(targetdata$dep_value,col="blue")
plot(density(targetdata$dep_value),col="blue")
skewness(targetdata$dep_value)

plot(targetdata$Date_of_Journey,targetdata$Price,col=c("green","red"),
     main="Qual Vs. Fees",font=10)

plot(targetdata$dep_value,targetdata$Price,col=c("green","red"),
     main="Qual Vs. Fees",font=10)
...
model2 = randomForest(ntrain$Price ~ .,data = ntrain,method="rf", ntree=1000, mtry=35, importance=TRUE, Proximity=TRUE, na.action=na.exclude)
print(model2)
...
table(is.na(p1))
check_na <- p1[rowSums(is.na(p1)) > 0,]
check_na
...
testdata = read.csv("pred.csv")
colnames(testdata) = gsub('-','_',colnames(testdata))

testdata$ad_Trujet = 0
testdata$ad_AI1_Short_layover = 0
testdata$ad_AI2_Long_layover = 0
testdata$ad_AIIn_flight_meal_not_included = 0
testdata$ad_AINo_check_in_baggage_included = 0
testdata$ad_AINo_info = 0
testdata$ad_AINo_Info = 0
testdata$ad_AIRed_eye_flight = 0

p2=predict(model2, testdata, na.action=na.pass)


write.csv(p2, "Submission_file.csv", row.names = FALSE)
